...
function gotoManageClass(classroomId) {
    ...
    searchStudentButton.addEventListener('click', async () => {
        ...
        try {
            ...
            const querySnapshot = await usersRef.where("email", "==", studentId + "@kkumail.com").get();
...
//
            // --- Event Listener for adding student (inside the modal) ---
            addStudentButtonModal.onclick = () => { // ใช้ onclick สะดวกกว่า
                addStudentToClass(studentId, studentData.name, studentUid); // Pass UID here
                addStudentModal.style.display = 'none'; // Close modal after adding
            };
...
// --- Add Student ---
async function addStudentToClass(studentId, studentName, studentUid) {
    try {
        ...
        const studentRef = classroomRef.collection('students').doc(studentUid); // Use UID
        ...
        // Add student to /classrooms/{cid}/students/{uid}
        await studentRef.set({
            studentId: studentId, // Keep the student ID for display
            name: studentName,
            status: true,
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            row.innerHTML = `
                ...
                <td>${studentData.status ? 'Active' : 'Inactive'}</td>
            `;
...
async function showCheckedInStudents(classroomId, checkinId) {
    try {
        ...
        snapshot.forEach(doc => {
            const studentData = doc.data();
            const listItem = document.createElement('li');
            listItem.textContent = `${studentData.studentId} - ${studentData.name}`;
